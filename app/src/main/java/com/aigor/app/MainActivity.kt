            ...
            try {
                ...
                val payload = JSONObject().apply {
                    ...
                    if (!bridgePub.isNullOrBlank()) {
                        ...
                        encResult = DevE2ee.encryptForBridge(payloadText, bridgePub, "aigor-app-chat", bridgeOtkId, nextCounter)
                        put("message", "")
                    ...
                    attachment?.let {
                        if (encResult != null) {
                            put("e2eeAttachment", DevE2ee.encryptAttachment(it.base64, encResult!!.responseKey, it.name, it.mime, "aigor-app-chat"))
                        } else {
                    ...
                    val assistantTextRaw = try {
                        ...
                        if (obj.has("e2eeReply") && encResult != null) {
                            ...
                            if (inCounter > 0 && inCounter <= lastIn) {
                                "[E2EE] Resposta descartada per counter no monotònic"
                            } else {
