...
def main():
    ...
    tmp = Path(tempfile.mkdtemp(prefix="e2ee-failed-decrypt-next-counter-dh-step-header-norm-smoke-"))
    ...
    mod = load_bridge(script_path, "bridge_failed_next_counter_dh_step_header_norm")
    sid = "failed-decrypt-next-counter-dh-step-header-norm"
    canonical = "hdr-next-counter-dh-norm"
    spaced = f"  {canonical}  "
    ...
    precheck_n = mod._ratchet_check_and_advance(sid, 2, _norm_header(spaced))
    ...
    try:
        mod.decrypt_real_envelope({
            ...
            "ad": "failed-counter-n-dh-step-header-norm",
            "counter": 2,
            "headerId": spaced,
        }, sid)
    except Exception:
    ...
    env_n_plus_1 = _build_valid_env(
        ...
        ad="success-counter-n-plus-1-dh-step-header-norm",
        plaintext="ok-n-plus-1-dh-header-norm",
    )
    pt, _, _, out_counter = mod.decrypt_real_envelope(env_n_plus_1, sid)

    after_success = _session_recv(mod, sid)

    previously_failed_n_still_accepted = mod._ratchet_check_and_advance(sid, 2, _norm_header(canonical))
    replay_n_reject_after_accept = not mod._ratchet_check_and_advance(sid, 2, _norm_header(spaced))

    ok = (
        precheck_n
        and failed_n
        and before == after_restore
        and pt == "ok-n-plus-1-dh-header-norm"
        and out_counter == 3
    ...
    print(json.dumps({
        ...
        "nextCounterAccepted": pt == "ok-n-plus-1-dh-header-norm" and out_counter == 3,
