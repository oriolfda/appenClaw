...
def main():
    ...
    tmp = Path(tempfile.mkdtemp(prefix="e2ee-failed-decrypt-next-counter-smoke-"))
    ...
    mod = load_bridge(script_path, "bridge_failed_then_next_counter")
    sid = "failed-decrypt-next-counter"
    header = "hdr-next-counter"
    ...
    env_n_plus_1 = _build_valid_env(
        ...
        counter=3,
        header_id=header,
        ad="success-counter-n-plus-1",
        plaintext="ok-n-plus-1",
    )
    ...
    print(json.dumps({
        ...
        "nextCounterAccepted": pt == "ok-n-plus-1" and out_counter == 3,
