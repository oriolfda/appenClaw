...
> "I want to use appenClaw as-is, without rebranding or rebuilding it. Use `https://github.com/oriolfda/appenClaw` as the canonical reference and follow `docs/APPENCLAW_AI_REPLICA.md` strictly in AS-IS mode. Do not rebrand or rebuild the APK unless necessary. Prepare the bridge, give me a working endpoint + token, explain whether nginx/firewall/domain setup is needed for external access, and tell me how to download and configure the `appenClaw-release.apk`."
...
> "I want to create a personalized assistant-app starting from appenClaw. Use `https://github.com/oriolfda/appenClaw` as the canonical reference and follow `docs/APPENCLAW_AI_REPLICA.md` strictly in personalized mode. Run interactive intake for name, icon, locale, theme, STT/TTS preferences, target agent, bridge port, and deployment model. Then personalize an appropriate working repository, build a release APK, prepare the dedicated bridge, and leave me clear install/use instructions."
